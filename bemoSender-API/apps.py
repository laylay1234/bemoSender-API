...
class bemosenderrrConfig(AppConfig):
    name = 'bemosenderrr'
    verbose_name = 'bemosenderrr'


    def ready(self):
        import bemosenderrr.signals  # noqa
        ...
        try:
            ...
            if not update_rates_instance:
                periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=e.key, task="bemosenderrr.tasks.update_rates", 
            ...
            e = Entry(schedule=900, name=f'Update Rates periodic task', app=app,
                        task="bemosenderrr.tasks.update_rates")
            e.save()
            key = e.key
            # Periodic task object to manage redbeat periodic tasks dynamically
            periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=key, task="bemosenderrr.tasks.update_rates", 
                    name=e.name, schedule=900, args=None
            )
        ...
        try:
            ...
            if not authorize_deposits_instance:
                periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=e.key, task="bemosenderrr.tasks.authorize_deposits", 
            ...
            e = Entry(schedule=deposits_schedule, name=f'Authorize Deposits Periodic Task', app=app,
                        task="bemosenderrr.tasks.authorize_deposits")
            e.save()
            key = e.key
            print(e.key)
            # Periodic task object to manage redbeat periodic tasks dynamically
            periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=key, task="bemosenderrr.tasks.authorize_deposits", 
                    name=e.name, schedule=deposits_schedule, args=None
            )
        ...
        try:
            ...
            if not check_deposits_instance:
                periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=e.key, task="bemosenderrr.tasks.check_deposits", 
                        name=e.name, schedule=deposits_schedule, args=None
                )
        except Exception as e:
            ...
            e = Entry(schedule=deposits_schedule, name=f'Check Deposits Periodic Task', app=app,
                        task="bemosenderrr.tasks.check_deposits")
            e.save()
            key = e.key
            print(e.key)
            # Periodic task object to manage redbeat periodic tasks dynamically
            periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=key, task="bemosenderrr.tasks.check_deposits", 
                    name=e.name, schedule=deposits_schedule, args=None
            )
        try:
            ...
            if not check_refunds_instance:
                periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=e.key, task="bemosenderrr.tasks.check_refunds", 
                        name=e.name, schedule=deposits_schedule, args=None
                )
        except Exception as e:
            ...
            e = Entry(schedule=deposits_schedule, name=f'Check Refunds Periodic Task', app=app,
                        task="bemosenderrr.tasks.check_refunds")
            e.save()
            key = e.key
            print(e.key)
            # Periodic task object to manage redbeat periodic tasks dynamically
            periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=key, task="bemosenderrr.tasks.check_refunds", 
                    name=e.name, schedule=deposits_schedule, args=None
            )
        try:
            ...
            e = Entry(schedule=60, name=f'Check Async Flinks Requests', app=app,
                        task="bemosenderrr.tasks.update_pending_sign_up_flinks")
            e.save()
            key = e.key
            print(e.key)
            # Periodic task object to manage redbeat periodic tasks dynamically
            periodic_task, created = PeriodicTasksEntry.objects.get_or_create(key=key, task="bemosenderrr.tasks.update_pending_sign_up_flinks", 
                    name=e.name, schedule=60, args=None
            )
