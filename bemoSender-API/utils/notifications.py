...
Language used for senderr push notifications depends on the user locale (user.locale)
"""
...
class NotificationsHandler():
    ...
    def get_tx_collect_ready_senderr_push(self, lang="FR", vars=[]):
        if len(vars) != 0:
            if lang == "FR":
    ...
    def get_tx_collect_bank_ready_receiver_sms(self, lang="FR", vars=[]):
        if len(vars) != 0:
            if lang == "FR":
                app_name = "bemosenderrr" # Can get changed at some point!
    ...
    def get_tx_collected_senderr_push(self, lang="FR", vars=[]):
        if len(vars) != 0:
            if lang == "FR":
    ...
    def get_tx_funding_required_senderr_push(self, lang="FR", vars=[]):
        if len(vars) != 0:
            if lang == "FR":
                if vars[3] == "0":
    ...
    def get_tx_funding_incactivity_senderr_push(self, lang="FR", vars=[]):
        if len(vars) != 0:
            if lang == "FR":
    ...
    def get_tx_delayed_senderr_push(self, lang="FR", vars=[]):
        if len(vars) != 0:
            if lang == "FR":
